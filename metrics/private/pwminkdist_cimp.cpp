...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (!mxIsNumeric(mxX1) || mxIsSparse(mxX1) || mxIsSparse(mxX2) || 
	mxIsComplex(mxX1) || mxIsComplex(mxX2))
    {
	mexErrMsgTxt("Only that X1 and X2 are both full real matrices are supported.");
    }

    if (mxGetM(mxX2) != d)
    {
	mexErrMsgTxt("X1 and X2 should have the same number of rows.");
    }

    if (mxGetClassID(mxX2) != cid)
    {
	mexErrMsgTxt("X1 and X2 should be of the same value type.");
    }
	...
	{
	    mexErrMsgTxt("When p is 1 or inf, only double, single, and int32 types are supported.");
	}
    }
    else
    {
	...
	{
	    mexErrMsgTxt("when p is a value other than 1 or inf, only floating point types are supported.");
	}
    }
